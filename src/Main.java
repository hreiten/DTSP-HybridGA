...
public class Main {
    ...
    private City[] cities; // city array (subject to dynamical movements)
    private City[] originalCities; // the original cities (not subject to change)
     ...
     * @param generations Number of generations in the GA
     */
    public Main(City[] cities, City[] originalCities, int populationSize, int generations) {
        this.cities = cities;
    ...
    private static Chromosome mutate(Chromosome original, City[] cityList, double mutationProb) {
        ...
        if (sampledProb <= mutationProb) {
            newCityIndexes = Mutation.mutateInversion(newCityIndexes);
        }
    ...
    private static boolean validatePopulation(Chromosome[] chromosomes) {
        for (Chromosome c : chromosomes) {
            if (Arrays.stream(c.getCityIndexes()).distinct().count() != c.getCityIndexes().length) {
                return false;
            }
        }
